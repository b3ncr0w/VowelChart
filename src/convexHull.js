...
(function (convexhull) {
    ...
    function makeHull(points, sensitivity) {
        var newPoints = points.slice();
        newPoints.sort(convexhull.POINT_COMPARATOR);
        return convexhull.makeHullPresorted(newPoints, sensitivity);
    }
    convexhull.makeHull = makeHull;
    // Returns the convex hull, assuming that each points[i] <= points[i + 1]. Runs in O(n) time.
    function makeHullPresorted(points, sensitivity) {
        if (points.length <= 1)
            return points.slice();
        ...
        for (var i = 0; i < points.length; i++) {
            ...
            while (upperHull.length >= 2) {
                ...
                if ((q.x - r.x) * (p.y - r.y) >= (q.y - r.y) * (p.x - r.x) * sensitivity)
                    upperHull.pop();
                else
                    break;
            }
